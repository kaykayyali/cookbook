...
export function recipeCardHTML(r, pantry, { currentUserSub = null, history = null } = {}) {
  ...
  return `
    <article class="card recipe-card card-fold-${e}" data-id="${esc(r._id)}" data-feedback="select" tabindex="0" aria-label="Open ${esc(r.name)} recipe">
      ...
      <div class="card-body">
        ...
        <div class="card-head">
          ...
          <div class="card-toolbar">
            ${canManage ? IconButton({ label: 'Edit', icon: 'edit', size: 'sm', danger: false, data: { action: 'edit', id: r._id, feedback: 'commit' } }) : ''}

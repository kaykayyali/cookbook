...
test('recipe entry keeps long name usable and exposes absolute move, skip, repeat, and servings controls', async () => {
  const { dom, mutations } = setup([{
    id: 'e1', date: '2026-07-14', type: 'recipe', recipeId: 'r1', targetServings: 4,
  ...
  for (const action of ['move-next', 'skip', 'repeat', 'servings-up']) {
    root.querySelector(`[data-action="${action}"]`).click();
    await Promise.resolve();
  }
  assert.deepEqual(mutations.map((item) => item.op), [
    'plan.upsert', 'plan.upsert', 'plan.upsert', 'plan.upsert',
  ]);
  assert.equal(mutations[0].payload.date, '2026-07-15');
  assert.equal(mutations[1].payload.status, 'skipped');
  assert.equal(mutations[2].payload.date, '2026-07-21');
  assert.equal(mutations[3].payload.targetServings, 5);
});
